...
function getAccountById(id) {
  ...
  if (!Number.isInteger(id) || id <= 0) {
	return `Invalid account ID. The ID must be a positive integer.`;
  }
...
function createAccount(newAccountId, newAccountOwner)
{
	...
	{
		console.log("Invalid value for account ID: The account ID must be a positive finite integer.");
	}

	if (typeof newAccountOwner !== "string" || newAccountOwner.trim() === "")
	{
		console.log("Invalid account owner: The account owner must be a non-empty string.");
	}
